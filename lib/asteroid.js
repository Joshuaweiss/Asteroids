(function() {
  ...
  Asteroid.prototype.colideBullet = function(bullet) {
    var asteroidOneDiff = this.pos.diff(window.Asteroids.gameview.game.ship.pos).withMag(this.radius + 35).rotate(Math.PI/2);
    var asteroidTwoDiff = this.pos.diff(window.Asteroids.gameview.game.ship.pos).withMag(this.radius + 35).rotate(-Math.PI/2);
    if (this.type < asteroidOptions.length - 1) {
      return [
        new Asteroid(
          {
            pos: this.pos.add(asteroidOneDiff),
            vel: asteroidOneDiff.multiplier(0.6),
            type: this.type + 1
          }
        ),
        new Asteroid(
          {
            ...
            vel: asteroidTwoDiff.multiplier(0.6),
            type: this.type + 1
          }
        )
      ]
